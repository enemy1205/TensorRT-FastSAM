...
class FastSam(object):
    ...
    def segment(self, bgr_img):
        ...
        result = postprocess(preds, inp, bgr_img,retina_masks,conf,iou)
        ...
        for i, mask_i in enumerate(masks):
            ...
            rand_color_ = (g, r, b)
            box = boxes[i]
            # cv2.rectangle(image_with_masks,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),rand_color_,3)
        cv2.imwrite("outputs/obj_trt.png", image_with_masks)
        

        # pil_image = Image.fromarray(img)
        
        # rgb_image = Image.fromarray(cv2.cvtColor(img,cv2.COLOR_BGR2RGB))

        # image_with_masks = segment_everything(rgb_image, result, input_size=self.imgsz[0])

        # image_with_masks.save(f"outputs/obj_segment_trt_rgb.png")
        # cv2.imwrite(f"outputs/obj_segment_trt.png", image_with_masks)
        return masks

if __name__ == '__main__':
    model = FastSam(model_weights="models/fast_sam_s.engine")
